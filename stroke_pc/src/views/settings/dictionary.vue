<template>
  <el-container>
    ...
    <el-main>
      <el-table ref="dictionaryTable" :data="tableData" style="width: 100%;" height="calc(100vh - 190px)">
        <el-table-column prop="code" label="标识" width="260" />
        <el-table-column prop="name" label="字典名称" width="180" />
        <el-table-column prop="itemcode" label="细项标识" width="80" />
        <el-table-column prop="itemname" label="细项名称" width="160" />
    ...
    <el-dialog v-loading="loading" :visible.sync="dialogVisible" :title="dialogTitle" center>
      <el-form ref="dialogForm" :model="dialogForm" :rules="rules" label-width="100px" class="demo-ruleForm">
        ...
        <el-form-item label="细项标识" prop="itemcode">
          <el-input v-model="dialogForm.itemcode" clearable />
        </el-form-item>
        <el-form-item label="细项名称" prop="itemname">
          <el-input v-model="dialogForm.itemname" clearable />
        </el-form-item>
        <el-form-item label="扩展" prop="extend">
          <el-input
            v-model="dialogForm.extend"
            type="textarea"
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        itemcode: [{ required: true, message: '请输入细项标识', trigger: 'blur' }],
        itemname: [{ required: true, message: '请输入细项名称', trigger: 'blur' }],
  ...
  methods: {
    ...
    getList() {
      settingsApi.dictionaryPage({ ...this.searchForm, ...this.pageInfo })
        .then(res => {
    ...
    resetDialogForm() {
      ...
      this.$set(this.dialogForm, 'itemcode', '')
      this.$set(this.dialogForm, 'itemname', '')
      this.$set(this.dialogForm, 'extend', '')
      this.$set(this.dialogForm, 'sort', '')
      this.$set(this.dialogForm, 'status', '')
      this.$set(this.dialogForm, 'describe', '')
    },
    submit() {
    ...
    add() {
      settingsApi.dictionarySave({ ...this.dialogForm })
        .then(res => {
          this.loading = false
          if (res.code === '0') {
    ...
    update() {
      settingsApi.dictionaryUpdate({ ...this.dialogForm })
        .then(res => {
          this.loading = false
          if (res.code === '0') {
    ...
    delete(id) {
      settingsApi.dictionaryDelete({ id: id })
        .then(res => {
          if (res.code === '0') {
