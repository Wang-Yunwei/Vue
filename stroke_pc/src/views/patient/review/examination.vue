<template>
  <div>
    <div>
      <el-descriptions v-for="(item,index) in examination" :key="index" class="desStyle" :title="item.examClass + item.examSubClass" :column="1" border>
        <el-descriptions-item label="报告时间" :label-style="widthValue">{{ item.reportTime }}</el-descriptions-item>
        <el-descriptions-item label="结论" :label-style="widthValue">{{ item.conclusion }}</el-descriptions-item>
        <el-descriptions-item label="检查所见" :label-style="widthValue">{{ item.description }}</el-descriptions-item>
      </el-descriptions>
    </div>
    <div v-if="!showEmpty">
