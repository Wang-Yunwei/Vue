<template>
  <div class="med-datetime-wrapper">
    <div class="date-part">
      <el-date-picker v-model="dateValue" type="date" v-bind="$attrs" format="yyyy-MM-dd" :picker-options="options"
        :default-value="defaultValue" value-format="yyyy-MM-dd" placeholder="请选择日期" :disabled="disabled"
        @change="onChange" @focus="onFocus" :readonly="readonly" />
    </div>
    <div v-if="type === 'datetime'" class="space" />
    <div v-if="type === 'datetime'" class="time-part">
      <el-time-picker v-model="timeValue" v-bind="$attrs" arrow-control :disabled="disabled" :readonly="readonly"
        :editable="editable" :clearable="clearable" :default-value="defaultValue" :format="timeFormat"
        :value-format="timeFormat" :picker-options="{
          format: timeFormat
        }" placeholder="时间" @change="onChange" @focus="onFocus" />
    </div>
  </div>
</template>
<script>
...
const __format__ = 'YYYY-MM-DD HH:mm'
export default {
  name: 'MedDatetime',
  directives: {
    fast: {
      inserted: (el, binding, vnode) => {
        ...
        if (vnode.componentOptions.tag === 'el-date-picker') {
          ...
          elm.addEventListener('focus', () => {
            ...
            var arr = [...document.querySelectorAll('.el-date-picker')].map(
              (e) => +window.getComputedStyle(e).zIndex || 0
            )
            var max = arr.length ? Math.max(...arr) : 0
            var pop = [...document.querySelectorAll('.el-date-picker')].find(
              (e) => (+window.getComputedStyle(e).zIndex || 0) === max
            )

            var timeInput = pop.querySelectorAll(
  ...
  methods: {
    ...
    formatTime(val, defaultVal = { hour: 0, minute: 0, second: 0 }) {
      if (/(^[1-9]\d*$)/.test(Number(val))) {
        if (val.length === 2) { return { hour: val > 23 ? 0 : val, minute: 0, second: 0 } } else if (val.length === 4) {
