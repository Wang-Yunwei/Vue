    ...
    <med-s-card class="bottom-card" :bordered="false">
      ...
      <div class="bottom-card-main">
        <ul class="list">
          <li v-for="(item, idx) in list" :key="idx">
            <div class="flex-c-b" v-if="item.label !== 'GCS评分'">
              <div class="flex-c-s">
            ...
            <div class="flex-c-b" v-else>
              <div class="flex-c-s">
                <span style="min-width: 100px;">{{ item.label }}</span>
                <div>
                  <div class="flex-c-s childDiv" v-for="(el, idd) in item.content" :key="idd">
                    <span class="childTitle">{{ el.label }}</span>
                    <med-s-radio
                      buttonType
                      :initialValue="el.initialValue"
                      :optionList="el.content"
                      v-model="el.checked"
                      @change="val => onChange(val, item.label, item, el)"
                    />
                  </div>
                </div>
              </div>
      ...
      <div class="bottom-card-tip">
        <div class="one flex-c-s">
          ...
          <span>
            注：RTS：1~6分为危重伤，病死率极高；7~11分为中度伤，病死率较高；12分为轻度伤，通常无死亡发生。
          </span>
        </div>
      </div>
    </med-s-card>
...
export default {
  ...
  methods: {
    ...
    initList(arrs) {
      ...
      this.list?.forEach(item => {
        arr?.forEach(el => {
          if (item.label === 'GCS评分') {
            let score = 0
            item.content.forEach(ell => {
              el.value.forEach(elll => {
                if (ell.label === elll.type) {
                  this.$set(ell, 'checked', elll.value[0].value)
                  this.$set(ell, 'score', elll.value[0].score)
                  this.$set(ell, 'initialValue', elll.value[0].value)
                }
                score += ell.score || 0
              })
            })
            this.$set(item, 'score', score)
          } else if (item.label === el.type) {
    ...
    onChange(val, title, item, el) {
      if (title === 'GCS评分') {
        const result = this.details?.some(ell => ell.type === item.label)
        const obj = el.content.find(ell => ell.value === val)
        const subResult = this.subDetail.some(ell => ell.type === el.SubName)
        if (!result) {
          if (!subResult) {
            this.subDetail.push({ type: el.SubName, value: [{ id: obj.ItemId, value: obj.ItemId, score: obj.Score }] })
          } else {
            this.subDetail = this.subDetail.map(item => {
              return {
                ...item,
                ...{ value: [{ id: obj.ItemId, value: obj.ItemId, score: obj.Score }] }
              }
            })
          }
          this.details?.push({ type: item.label, value: this.subDetail })
        } else {
          if (!subResult) {
            this.subDetail.push({
              type: el.SubName,
        ...
        this.$set(el, 'score', obj.Score)
        let total = 0
        item.content.length &&
          item.content.forEach(cc => {
            total += Number(cc.score) || 0
          })
        ...
        const finalScore = [
          ...
          [9, 12],
          [13, 15]
        ].findIndex(r => total >= r[0] && total <= r[1])

        this.$set(item, 'score', finalScore)
  ...
  computed: {
    ...
    changed() {
      return this.list.some(el => {
        if (el.label !== 'GCS评分') {
          return el.checked !== el.initialValue
        }
        return el.content?.some(item => item.checked !== item.initialValue)
      })
    }
  },
  mounted() {
