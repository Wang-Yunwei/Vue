<template>
  <div class="med-tn-field-wrapper">
    ...
    <van-list
      :class="disabled ? 'display_list_disabled' : 'display_list'"
      readonly
    >
      <div
        v-for="(item, index) in labRecords"
        :key="item.id"
        class="display_list-row"
      >
        <van-cell>
          ...
          <van-swipe-cell>
            <van-cell class="med-tn-row-cell">
              ...
              <med-lab-result-field
                ...
                v-labResult="{ scale: 3, min: 0, max: 99999999 }"
                required
                @confirm="onConfirm(item)"
              >
                <template #label>
...
.med-tn-row {
  ...
  /deep/
    .med-datepicker-wrapper:nth-last-child(1)
    .med-hint-field--bottom::after {
    border: none;
  }
}
</style>
