<template>
  <div class="med-text-field-wrapper">
    ...
    >
      <template #right-icon>
        <van-icon name="volume" @click="inputVoice" v-if="voiceShowed" />
    ...
    >
      <van-empty
        ...
        description="暂无数据"
        v-if="show && !selectOptions.length"
      />
...
<script>
import setting from '@/mixins/settingMixin'
  ...
  methods: {
    ...
    onVoiceInputEnd() {
      const msg = this.$bridge.onVoiceInputEnd(res => {
        this.fieldValue = res
        this.onConfirm()
        console.log('onVoiceInputEndcallback', res)
      })
