  ...
  <div class="med-ecg-edit-wrapper">
    ...
    <div class="med-ecg-uploader-info">
      ...
      <!-- <med-list-select-field
        label="异常"
        v-if="enabledReason && ecg.ecgDiagnosis === '0'"
        v-model="ecg.ecgException"
        :select-options="exceptionOptions"
      /> -->
    </div>
...
export default {
  ...
  methods: {
    ...
    async doUploadFile(file) {
      ...
      console.log('上传', file, file.File, typeof file)
      try {
    ...
    'ecg.ecgTime': {
      ...
      handler(newVal) {
        if (
          newVal &&
          this.ecg.diagTime &&
          this.$moment(newVal).isAfter(this.$moment(this.ecg.diagTime))
        ) {
          this.$notify({
            type: 'warning',
            message: '心电图时间应早于心电图诊断时间'
          })
        }
      },
      deep: true
    ...
    'ecg.diagTime': {
      ...
      handler(newVal) {
        if (
          newVal &&
          this.ecg.ecgTime &&
          this.$moment(this.ecg.ecgTime).isAfter(this.$moment(newVal))
        ) {
          this.$notify({
            type: 'warning',
            message: '心电图时间应早于心电图诊断时间'
