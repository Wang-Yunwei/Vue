        ...
        <van-cell class="card-item">
          ...
          <div v-for="ecg in getEcgFiles(item)" :key="ecg">
            <van-image
              ...
              :src="ecg"
              fit="scale-down"
              @click="onPreviewRecord(item)"
            />
          </div>
          <van-empty
            v-if="getEcgFiles(item).length == 0"
...
style="margin-left:6px"
                  >完成时间：{{ getFormatTime(item.ecgTime) }}</span
                >
              </div>
              <div class="infoTime">
                <van-icon name="clock-o" />
                <span
style="margin-left:6px"
                  >诊断时间：{{ getFormatTime(item.diagTime) }}</span
                >
              </div>
            </div>
            <!-- <span class="infoE">
