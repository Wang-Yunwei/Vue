<template>
  <div class="task-preliminary-diagnosis-wrapper">
    <van-collapse v-if="emId && patientInfo.outpatientNo" v-model="exhibit" @change="collapseChange">
      <van-collapse-item title="基本信息" name="exhibit1">
        <div class="center">
          ...
          <p>
            <span>性别</span>
            <span>
              {{ conversionOne(triage.sex, dictionary.genderDict) }}
            </span>
          </p>
          <p>
            <span>年龄</span>
          ...
          <p>
            <span>分诊级别</span>
            <span
              >{{ triage.resetLevel }} {{ triage.resetLevel ? '级' : '' }}</span
            >
          </p>
          <!-- <p>
            <span> 三无患者:</span>
...
export default {
  ...
  data() {
    return {
      diagnosisTwo: '',
      // hospitalId: this._permission().user.hospitalId, // 获取用户登入医院id
