          ...
          <p>
            ...
            ><span>{{
              conversionOne(
                ...
                dictionary.genderOpts
              )
            }}</span>
          </p>
          <p>
            <span>年龄</span><span>{{ firstAid.patientBaseInfoDto.age }}</span>
          </p>
          <p>
            <span>民族</span
            ><span>{{
      ...
      <van-collapse-item title="诊断处理" name="exhibit7">
        <div class="center">
          ...
          <p>
            ...
            <span>{{
              conversionOne(
                firstAid?.diagnosticProcessingDto?.rescueChangeResult,
  ...
  methods: {
    async getPreHospitalFirstAidRecord() {
      this.$api.preHospitalPush
        .getMedicalRecord({ ...this.query, missionId: this.missionId })
        .then(res => {
          if (res.code === 0) {
    ...
    handleBind() {
      this.$router.push({
        ...
        query: {
          from: 'import',
          patientHistory: { ...this.$route.query.patientHistory }
        }
      })
    },
    imagePreview(item) {
