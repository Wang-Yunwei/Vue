...
const threeDaysRange = () => [
  moment().subtract(2, 'days').format(formatStr),
...
export default {
  ...
  data() {
    ...
    return {
      ...
      query: {
        ...
        fromDate: startOfDay.subtract(3, 'days').format(formatStr),
        ...
        shortcuts: [
          {
            text: '近三天',
            onClick: (picker) => {
              picker.$emit('pick', [getStart(3), new Date()])
              this.handleSearch()
            }
          },
          {
            text: '最近一周',
            onClick: (picker) => {
              picker.$emit('pick', [getStart(7), new Date()])
              this.handleSearch()
            }
          },
          {
  ...
  mounted() {
    // console.log(storage.get('dicData'), 'storage.get(dicData)')
  },
  methods: {
    onTypeChange(type) {
      if (type === '1') {
    ...
    showModal() {
      ...
      this.query.patientName = this.patientName
      this.handleSearch()
    },
    hideModal() {
    ...
    handleSearch(curPage = 1) {
      ...
      this.query.hospitalArea = this.hospitalArea
      ...
      this.$confirm(
        ...
        .then(() => {
          getEmIdPatient({
            ...
            .then((res) => {
              if (res.status === 200) {
                if (res.result && res.result.isRegister) {
                  this.$message.error('该患者已被关联')
                } else {
                  this.$emit('bind', { ...item, ...{ type: this.query.type }}, this.hideModal)
                }
              }
            })
        })
