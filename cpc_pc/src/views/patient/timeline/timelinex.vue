<template>
  <ul class="timeline-wrapper">
    <li v-for="group in dateGroups" :key="group.key" class="date-row flex-c-s">
      <div class="date">
        <div>{{ group.year }}</div>
        <p>{{ group.date }}</p>
      </div>
...
export default {
  ...
  async mounted() {
    const { result } = (await getTimeline({ registid: this.id })) || {
      autoTimesList: []
    }
    this.timeline = result.autoTimesList
    this.genGroup()
  },
  methods: {
    genGroup() {
      const groups = []
      this.timeline.forEach(({ id, timeValue, nodeName }) => {
