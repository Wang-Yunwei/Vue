<template>
  <div>
    <el-form :inline="true" :model="searchparam" size="mini">
      ...
      <el-form-item label="关键质控类型">
        <el-checkbox-group v-model="searchparam.patientType" @change="onSubmit()">
          <el-checkbox label="D2W" />
          <el-checkbox label="FMC2ECG" />
          <el-checkbox label="FMC2W" />
          <el-checkbox label="D2N" />
        </el-checkbox-group>
      </el-form-item>
    </el-form>
  </div>
...
export default {
  ...
  data() {
    return {
      ...
      searchparam: {
        daterange: [],
        patientType: ['D2W', 'FMC2ECG', 'FMC2W', 'D2N']
      },
      pickerOptions: {
        disabledDate(time) {
  ...
  methods: {
    ...
    onSubmit() {
      const typeCopy = []
      if (this.searchparam.patientType.includes('D2W')) {
        typeCopy.push('D2W')
      }
      if (this.searchparam.patientType.includes('FMC2ECG')) {
        typeCopy.push('FMC2ECG')
      }
      if (this.searchparam.patientType.includes('FMC2W')) {
        typeCopy.push('FMC2W')
      }
      if (this.searchparam.patientType.includes('D2N')) {
        typeCopy.push('D2N')
      }
