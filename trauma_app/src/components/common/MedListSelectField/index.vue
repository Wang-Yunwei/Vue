<template>
  <div class="med-list-select-wrapper">
    <med-hint-field :label="label" @click="onFieldClick" readonly v-bind="$props" value="">
      <template #right-icon>
        <van-icon name="arrow" class="select-icon" />
      </template>
      <template #input-part v-if="!showDownList && labelList && labelList.length > 0">
        <div class="med-list-label-wapper">
          <div :class="['med-select-label-item', { 'item-disable': disabled }]" v-for="(item, index) in labelList"
    ...
    <van-list class="down-list" v-if="showDownList">
      <van-cell v-for="item in labelList" :key="item">

        <span :style="disabled ? 'color: rgb(154, 154, 154)' : ''">{{ item }}</span>
        <template #right-icon>
          <van-icon name="delete-o" :color="disabled ? 'rgb(154, 154, 154)' : 'red'" @click.stop="removeItem(item)" />
        </template>
      </van-cell>
    </van-list>
    <med-popup-container v-model="show" :title="label" get-container="body" position="right" @return="onReturnClick"
      scroll>
      <div @click.stop>
        <van-checkbox-group v-model="selectedOptions">
          <van-cell-group>
            <van-cell v-for="(item, index) in selectOptions" :key="item.value" :value="item.label" clickable
        ...
        <van-field v-if="showOther" v-model="otherContent" label="其他内容" maxlength="66" placeholder="请填写" />
      </div>
    </med-popup-container>
  </div>
</template>
