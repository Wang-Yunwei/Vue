  ...
  <div class="med-select-wrapper" @click="handleShowList">
    <div class="med-select-value flex-c-s" v-if="value">
      ...
      <template v-if="multiple">
        <div class="med-select-label-item" v-for="label in selectedLabels" :key="label"
          :style="disabled ? 'background-color: rgb(200, 201, 204);color: rgb(255,255,255); ' : 'background-color: rgb(228, 232, 255);color: #5e74fe;'">
          {{ label }}
        </div>
      </template>
    ...
    <div class="med-select-arrow">
      <slot name="icon" v-if="showArrow">
        <van-icon :name="openInPage ? 'arrow' : 'arrow-down'" color="white" />
      </slot>
    </div>
    <van-action-sheet cancel-text="取消" class="med-select-options" v-if="!disabled && !readonly && !openInPage"
      :duration="0.12" get-container="body" v-model="show" :actions="show ? actions : []" @select="handleSelect"
      @click.stop :description="multiple ? '(多选)' : ''">
      <van-empty :image="require('@assets/image/empty.png')" description="暂无数据" v-if="show && !actions.length" />
    </van-action-sheet>
    <van-popup v-model="show" :duration="0.15" get-container="body" position="right"
      v-if="!disabled && !readonly && openInPage" @click.stop>
      <van-empty :image="require('@assets/image/empty.png')" description="暂无数据" v-if="show && !actions.length" />
      <div class="med-select-page-wrapper">
        <med-header :title="pageTitle" @backClick="show = false" />
        <van-checkbox-group v-model="checkboxValue">
          <van-cell-group>
            <van-cell clickable v-for="(item, index) in options" :key="item.value" :title="item.label"
              @click="toggle(index)">
              <template #right-icon>
