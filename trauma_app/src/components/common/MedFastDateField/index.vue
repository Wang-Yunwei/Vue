<template>
  <div class="med-fast-date-field-wrapper">
    <med-hint-field hintType="" v-model="childTime">
      <template #right-part>
        <van-button :type="childTime ? 'primary' : 'warning'" @click.stop="onClick">
          {{ childTime ? "重新采集" : "采集" }}
        </van-button>
      </template>
      <template #input>
        <div :class="'input-field' + (required ? ' input-field-required' : '')">
          <div>{{ label }}</div>
          <med-date-picker v-bind="$props" hintType="" placeholder="请填写时间" v-model="childTime" clear-icon="clear" @confirm="onConfirm" :border="false"></med-date-picker>
        </div>
      </template>
    </med-hint-field>
  </div>
  ...
  methods: {
    onConfirm(val) {
      ...
      if ('object' === typeof val) {
        formattedDate = this.$moment(val).format(
          this.formatter || 'YYYY-MM-DD HH:mm'
        )
      }
      this.$emit('_change', formattedDate)
      this.$emit('confirm', formattedDate)
      if (this.voicePrompt && formattedDate) {
    ...
    onClick() {
      ...
      if (!this.childTime) {
        this.childTime = this.$moment().format(
          this.formatter || 'YYYY-MM-DD HH:mm'
        )
