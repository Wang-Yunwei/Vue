<template>
  <div class="med-select-wrapper">
    <div class="med-select-value flex-c-s" v-if="value" @click="handleShowList">
      <!-- {{ selectedLabels }} -->
    ...
    <div class="med-select-placeholder" v-if="!value" @click="handleShowList">
      {{ placeholder }}
    </div>
    <div class="med-select-arrow" @click="handleShowList">
...
export default {
  ...
  methods: {
    toggle(index) {
      ...
      if (this.multiple || !instance.checked) {
        instance.toggle()
      }
      if (instance.checked) {
      ...
      // 单选自动关闭打开的页面
      if (this.isOpenInNewPage && !this.multiple) {
        this.show = false
      }
    },
    handleShowList() {
