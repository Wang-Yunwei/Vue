<template>
  <div class="task-preliminary-diagnosis-wrapper">
    <van-collapse v-if="!showEmpty" v-model="exhibit" @change="collapseChange">
      <van-collapse-item title="基本信息" name="exhibit1">
        <div class="center">
          ...
          <p>
            <span>年龄</span>
            <span v-if="firstAid.patientBaseInfoDto.age">
              {{ firstAid.patientBaseInfoDto.age }} 岁
            </span>
          </p>
          <p>
            <span>民族</span>
          ...
          >
            ...
            <p>
              ...
              <span>
                {{ item.consciousness }}
                <!-- {{
                  conversionOne(item.consciousness, dicData.phepConsciousnessOpts)
                }} -->
              </span>
            </p>
          </van-cell>
      ...
      <van-collapse-item title="辅助检查" name="exhibit5">
        <div class="center">
          ...
          >
            <van-swipe-cell
              v-for="(item, index) in firstAid.supplementaryExaminationDto?.ecg"
              :key="index"
            >
              <van-cell>
                <van-image
          ...
          <p>
            <span>血糖</span>
            <span>{{ firstAid.supplementaryExaminationDto.bloodglucose }}</span>
          </p>
          <p class="titleText">
            POCT
          </p>
        ...
        <div class="center">
          ...
          <p>
            ...
            <span>
              ...
              <!-- {{
                conversionOne(
                  firstAid.diagnosticProcessingDto.conditionGrade,
                  dicData.conditionGradeOpts
                )
              }} -->
            </span>
          </p>
          <p>
            <span>急救措施</span>
            <span v-if="firstAid?.medPhepMeasuresEntityList.length > 0">
              <div
          ...
          <p>
            ...
            <span>
              ...
              <!-- {{
                conversionOne(
                  firstAid.medicalRecordInfoDto.treatmentResult,
                  dicData.rescueChangeResultOpts
                )
              }} -->
            </span>
          </p>
...
export default {
  ...
  computed: {
    ...
    ...mapGetters(['hospitalId', 'curHospitalArea', 'dicData'])
  },
  methods: {
    // 查询急救信息
    getPreviewList() {
      this.$api.preview
        .getfirstAidList(this.emId, this.infoComingType, this.hospitalAndArea())
        .then(({ result }) => {
          if (result && Object.keys(result)?.length) {
            this.firstAid = { ...this.firstAid, ...result }
          } else {
            this.showEmpty = true
