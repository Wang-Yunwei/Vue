    ...
    <van-collapse v-model="exhibit" @change="collapseChange">
      <van-collapse-item title="基本信息" name="exhibit1">
        <div class="center">
          ...
          <p>
            <span>民族</span>
            <span>{{ firstAid.patientBaseInfoDto.nation }}</span>
          </p>
          <p>
            <span>联系电话</span>
      ...
      <van-collapse-item title="诊断处理" name="exhibit7">
        <div class="center">
          ...
          <p>
            <span>病情判断</span>
            <span>{{ firstAid.medicalRecordInfoDto.conditionGrade }}</span>
          </p>
          <p>
            <span>急救措施</span>
  ...
  methods: {
    async getPreHospitalFirstAidRecord() {
      this.$api.preHospitalPush
        ...
        .then(res => {
          if (res.status === 200) {
            this.firstAid = { ...this.firstAid, ...res.result }
            console.log('推送列表-急救详情', this.firstAid)
          }
        })
    },
    handleBind() {
    ...
    $route: {
      ...
      handler(val) {
        ...
        if (this.$route.path === '/gc/preHospitalPreview') {
          this.getPreHospitalFirstAidRecord()
        }
      },
      deep: true
    }
