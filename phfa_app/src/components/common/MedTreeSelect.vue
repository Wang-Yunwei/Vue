    ...
    treeOptions() {
      const loop = arr => {
        return arr.map(el => {
          return {
            ...el,
            selected: el.value === this.value
          }
        })
      }
      return loop(this.options)
    },
    isOpenInNewPage() {
      return (
    ...
    selectedItems() {
      ...
      if (Array.isArray(this.formatedValue)) {
        this.formatedValue.forEach(val => {
          const matched = findInOpts(val)
          if (matched) {
            items.push(matched)
          }
        })
      } else {
        const matched = findInOpts(this.formatedValue)
        if (matched) {
          items.push(matched)
        }
      }
