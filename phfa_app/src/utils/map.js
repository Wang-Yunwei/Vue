...
function onLocation(data) {
  console.log('gps定位结果:', typeof data, data)
  const { resolve, reject } = promiseCallbacks
  if (typeof data === 'string' && data.startsWith('{') && data.endsWith('}')) {
    const json = JSON.parse(data)
    const result = wgs84tobd09(json.longitude, json.latitude)
    if (resolve) resolve(result)
  } else {
    if (reject) reject(new Error('获取定位失败! ' + data))
  }
  Bridge.callhandler('stopGPS', {})
  clearTimeout(overtime)
}

Bridge.registerHandler('onLocation', onLocation)
...
export function getCurLocation() {
  ...
  return new Promise((resolve, reject) => {
    if (isProd) {
      ...
      Bridge.callhandler('startGPS', {})

      overtime = setTimeout(() => {
        // Bridge.callhandler('stopGPS', {})
        reject(
          new Error('获取定位超时,请确认打开系统定位并准许APP使用定位权限')
        )
      }, 10000)
    } else {
      resolve({ longitude: 120.788018, latitude: 31.341061 }) // 开发环境默认公司门口
    }
  })
}
